...
class APICaller {
    ...
    func search(with query: String,page: Int,completion: @escaping (Result <[Image], Error>) -> Void) {
        ...
        guard let url = URL(string: "\(Constants.baseURL)q=\(query)&google_domain=google.com&tbs=itp%3Aphotos%2Cisz%3Al&tbm=isch&ijn=\(String(page))&device=mobile&api_key=\(Constants.API_KEY)") else {return print("Не прошел адрес")}
        print(url)
